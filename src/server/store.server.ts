...
ensureColumn(
	"host_sessions",
	"system_prompt",
	"ALTER TABLE host_sessions ADD COLUMN system_prompt TEXT",
);
...
const insertSessionStatement = database.query(
	`INSERT INTO host_sessions (
		id, provider, title, cwd, pinned, archived, status, status_detail_json, provider_session_ref, pid, imported, project_id, model, effort, system_prompt, permission_mode, allowed_tools_json, queued_input_count, usage_json, active_usage_json, last_event_sequence, create_time, update_time
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
);
...
const updateSessionStatement = database.query(
	`UPDATE host_sessions
		SET status = ?, status_detail_json = ?, provider_session_ref = ?, pid = ?, title = ?, pinned = ?, archived = ?, project_id = ?, model = ?, effort = ?, system_prompt = ?, permission_mode = ?, allowed_tools_json = ?, queued_input_count = ?, update_time = ?
		WHERE id = ?`,
);
...
export const sessionStore = {
	...
	updateSession(sessionId: string, update: SessionUpdate) {
		...
		const next: HostSession = {
			...
			systemPrompt: update.systemPrompt === undefined ? current.systemPrompt : update.systemPrompt,
